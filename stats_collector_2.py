...
import requests
flow_pusher_url = "http://129.7.98.24:8080/wm/staticentrypusher/json"
    ...
    for switch in switch_details_json:
        ...
        for port in port_details_json['port_reply'][0]['port']:
            port_dict[port['port_number']] = {'bandwidth_util': 0, 'time': int(time.time()),
                                              'tx': int(port['transmit_bytes']), 'rx': int(port['receive_bytes']),'link_util':-10000}
...
if __name__ == "__main__":
    ...
    while(1):
        #print switch_dict
        #print "Yes"
        fcpath = route_manager.FCpath(switch_dict,0,1)
        #print fcpath
